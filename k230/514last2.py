...
class SegmentationApp(AIBase):
    def __init__(self, kmodel_path, labels, model_input_size,
        ...
        # 【全自动标定基准】：赛场实际瓶子的真实高度和直径（请根据实际情况修改）
        ...
        self.last_D, self.last_H, self.last_L = 0.0, 0.0, 0.0
    ...
    # ========================================================
    # 频率聚类滤波算法：选出最频繁且较低的稳定液位，抵抗水泡跳动
    # ========================================================
    def get_frequent_lowest_L(self, data):
        if not data: return 0.0
        freq_map = {}
        for v in data:
            bin_key = round(v * 2) / 2.0 # 以 0.5cm 为步长进行聚类
    ...
    def measure_liquid_level(self, img, x, y, w, h):
            ...
            for dx in [-6, -3, 0, 3, 6]:
                ...
                if 0 <= img_cx < img_w and 0 <= img_cy < img_h:
                    try:
                        if len(img.shape) == 3:
                            if img.shape[0] == 3:
                                r = img[0, img_cy, img_cx]
                                g = img[1, img_cy, img_cx]
                                b = img[2, img_cy, img_cx]
                            else:
                                r = img[img_cy, img_cx, 0]
                                g = img[img_cy, img_cx, 1]
                                b = img[img_cy, img_cx, 2]
        ...
        if prev_luma < 0: return bottom_y

        for row in range(scan_start - 1, scan_end, -1):
            curr_luma = get_luma(row)
            if curr_luma < 0: continue

            diff = abs(curr_luma - prev_luma)
            if diff > max_diff and diff > 4.0:
                max_diff = diff
                liquid_y = row
    ...
    def _do_sample(self):
        ...
        # 【全自动同步校准】在第一次采样(50cm)时计算当前镜头的 Kh 与 Kw
    ...
    def _handle_command(self, cmd):
        ...
        if cmd == "CAL_START":
            ...
            self.state, self.meas_buffer_D, self.meas_buffer_H, self.meas_buffer_L = STATE_MEASURING, [], [], []
        ...
        if seg_res[0]:
            try:
                ...
                if best_bottle:
                    ...
                    # 2. 根据表面距离，换算出实际物理高度和物理宽度
                    final_H = self.compute_H(safe_h, avg_d_surface)
                    physical_width = safe_w * avg_d_surface * self.k_W

                    # 3. 计算中心距离（表面距离 + 物理半径补偿）
                    physical_radius = physical_width / 2.0
                    final_D = avg_d_surface + physical_radius
                    ...
                    # 更新结果与上屏
                    self.last_D, self.last_H, self.last_L = final_D, final_H, final_L
                    info_text = "D:%.1f H:%.1f L:%.1f W:%.1f" % (final_D, final_H, final_L, physical_width)
                    ...
                    if self.state == STATE_MEASURING:
                        ...
                        if self.meas_total_frames > 150:
                            ...
                            self.meas_buffer_D.clear(); self.meas_buffer_H.clear(); self.meas_buffer_L.clear()
            except: pass
    ...
    try:
        while True:
            ...
            with ScopedTiming("total", 0):
                ...
                # AI 每隔一帧运算一次，降低芯片负担防死机
                if frame_count % 2 == 0:
                    last_seg_res = seg.run(img)

                # 画图与测距每一帧都执行，保证画面流转跟手
                seg.draw_result(pl, img, last_seg_res, uart)

                # 测量结束，执行串口数据统一打包下发（含 D, H, L）
                if seg.meas_done:
                    result_str = "D:%.1f,H:%.1f,L:%.1f\n" % (seg.last_D, seg.last_H, seg.last_L)
                ...
                time.sleep_ms(10) # 给系统一点喘息时间，防止彻底卡死
