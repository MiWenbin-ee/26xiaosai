...
def init_uart2():
    try:
        fpioa = FPIOA()
        fpioa.set_function(5, FPIOA.TX0)
        fpioa.set_function(6, FPIOA.RX0)
...
class SegmentationApp(AIBase):
    ...
    def recognize_color(self, img, x, y, w, h, liquid_y):
        ...
        if core_bottom <= core_top or core_right <= core_left:
            return 0
        ...
        # 0:无色 1:红 2:绿 3:蓝 4:黄 5:黑 6:白 7:紫
            ...
            for cx in range(core_left, core_right, step_x):
                ...
                if 0 <= img_cx < img_w and 0 <= img_cy < img_h:
                    try:
                        ...
                        # 极具鲁棒性的颜色分类器
                        cid = 0
                        if v_val < 25: cid = 5 # 极暗 -> 黑色/酱油/可乐
                        elif s_val < 25 and v_val > 60: cid = 6 # 饱和度极低且亮 -> 白色/牛奶
                        elif s_val < 30: cid = 0 # 饱和度低 -> 无色纯水
                        else:
                            # 依靠色相 (H) 分配颜色
                            if h_val <= 15 or h_val >= 330: cid = 1 # 红
                            elif 15 < h_val <= 75: cid = 4 # 橙/黄 -> 算作黄
                            elif 75 < h_val <= 165: cid = 2 # 绿
                            elif 165 < h_val <= 270: cid = 3 # 蓝
                            elif 270 < h_val < 330: cid = 7 # 紫

                        color_votes[cid] += 1
                    except: pass

        # 选出票数最多的颜色作为最终判断
        best_cid = max(color_votes, key=color_votes.get)
        return best_cid
        ...
        if seg_res[0]:
            try:
                ...
                if best_bottle:
                    ...
                    final_D = avg_d_surface - (self.REAL_BOTTLE_WIDTH / 2.0)
                    ...
                    if self.state == STATE_MEASURING:
                        ...
                        if self.is_bottle_in_center(safe_x1, safe_y1, safe_w, safe_h):
                            ...
                            self.meas_buffer_C.append(color_id) # 记录测量的颜色
